...
Three upstream sibling packages needed fixes. Two shipped; one partial.
...
### Partial — blocks defipy 2.0.0 PyPI push

- **`uniswappy 1.7.8`** was intended to remove `import pytest` from `UniV3Utils.py:28` (or :29 — versions of the PROJECT_CONTEXT.md list the line differently). **The 1.7.8 wheel on PyPI still has the bug.** Confirmed during clean-venv verification: `pip install defipy` in an empty venv pulls `uniswappy 1.7.8` from PyPI and fails on import because the `import pytest` line survived the release build.

  Root cause unclear — either the fix was never committed, was made on a different branch, or the build picked up a stale wheel. Needs a fresh **`uniswappy 1.7.9`** release with the fix verified in the built wheel *before* uploading (check: `unzip -p dist/uniswappy-1.7.9-py3-none-any.whl uniswappy/utils/tools/v3/UniV3Utils.py | grep -c "import pytest"` should return 0).

  Until then, `pip install defipy==2.0.0` from PyPI would fail on import. Editable installs pointing at the worktree work fine because they bypass the installed siblings' `packages=[...]` and import from source directly.

**Gate for PyPI push:** fresh venv `pip install defipy` succeeds, imports resolve, the end-to-end smoke test from `DAY_3_BRIEF.md` passes. Until then, defipy 2.0.0 is tagged locally only.

Other post-push follow-ups:
- Bump defipy's `install_requires` to pin `uniswappy>=1.7.9` once it's out. `balancerpy>=1.1.0` and `stableswappy>=1.1.0` pins can land now.
- The 1.7.8 slot on PyPI is burned. Future users installing `uniswappy==1.7.8` get the broken wheel. There's no remediation for that slot; the fix lives at 1.7.9+.
...
| Clean-venv install works | broken (missing sub-packages + upstream bugs) | local: yes; PyPI: pending uniswappy 1.7.9 |
...
- **v2.0.0 tagging:** pending Day 4 editorial + uniswappy 1.7.9
- **PyPI push:** gated on uniswappy 1.7.9
- **ReadTheDocs v2 reorg:** Day 5-6
- **Public launch (Phase 3):** months out, paired with defipy.org

Working branch at 629 tests passing. Zero regressions against v1.2.0. Editable install fully functional; clean-venv PyPI install gated on upstream fix.
