...
DeFiPy is a Python SDK for DeFi analytics, simulation, and autonomous agents. Built from mathematical first principles with hand-derived AMM formulas. Current version: **1.2.0** (on PyPI).
...
**Seven primitives shipped as of the 1.2.0 working branch:**
...
Full suite: **159 tests passing** (primitives + fixture smoke tests). The full 19-primitive inventory and LP-question mapping lives in `doc/execution/DEFIMIND_TIER1_QUESTIONS.md`. Authoring conventions (file layout, style, test coverage, `__init__.py` wiring) are in `doc/execution/PRIMITIVE_AUTHORING_CHECKLIST.md`.
...
- **Numeraire convention: token0.** All position values, fees, and TVL figures are expressed in token0 units unless explicitly stated. Callers can re-denominate as needed. Enforced across AnalyzePosition, SimulatePriceMove, FindBreakEvenPrice, CheckPoolHealth, DetectRugSignals.
...
**Working-branch state: 159 tests passing.**
...
# Rug-signal detection (composes over CheckPoolHealth)
...
1. Verify 1.2.0 working-branch state: `pytest python/test/primitives/ -v` should show 159 passing.
2. Pick primitive #8 from the candidate list below.

### Primitive #8 candidates, with reasoning

**Strongest lean: `EvaluateRebalance` (reduced)** (P2, optimization/).
- Highest demonstration value remaining: actually chains AnalyzePosition + CalculateSlippage + FindBreakEvenPrice into a single decision primitive. DetectRugSignals showed composition over one primitive; EvaluateRebalance shows chaining three.
- Answers a real LP question ("should I rebalance this position now?")
- The "reduced" framing sidesteps the OptimalDepositSplit dependency — pass ticks directly, compare current-vs-exit, defer the optimal-deposit math to when OptimalDepositSplit actually ships
- Needs an explicit decision on the recommendation rule (when does it say "rebalance" vs "hold" vs "marginal"?) — flag this early in the session
- Estimated ~60 min

**Second choice: `DetectFeeAnomaly`** (P4, pool_health/).
- Natural pair with DetectRugSignals, same category, roughly same shape
- Answers Q7.3: compares theoretical fee output against actual. Needs a clear picture of how fees are computed vs. reported in V2/V3
- Smaller scope than EvaluateRebalance, but lower demonstration value — ships another threshold primitive, not a chaining one
- Estimated ~45 min

**Third choice: `AggregatePortfolio`** (P3, portfolio/).
- N × AnalyzePosition + aggregation. Needs a multi-pool fixture extension in `conftest.py`
- Creates a new category (`portfolio/`)
- Touches more scaffolding than the other candidates but straightforward math
- Estimated ~60 min including fixture work

**Fourth choice: `OptimalDepositSplit` V2-only** (P2, optimization/).
- V2 has a clean closed form (Uniswap V2 zap-in quadratic); V3 is non-trivial
- Sessions 2026-04-18 and 2026-04-21 both punted this because V3 needs a deliberate "raise vs. tick-walk" decision
- If taken on, make the V3 stance explicit up front; raise with a clear message is acceptable
- Estimated ~45 min V2-only, several hours for full V3

**Avoid for now: `AssessLiquidityDepth`** (P1, risk/).
- Genuinely hard — requires V3 tick-walking logic not written anywhere in the codebase yet
- Worth doing but deserves a focused fresh session
- Would also be the natural place to solve the tick-crossing gap flagged in the SolveDeltasRobust backlog entry
...
- **P3 — Comparison**: `CompareProtocols`, `CompareFeeTiers`, `AggregatePortfolio`
...
### Decision heuristics for picking the next primitive (general, beyond #8)
...
2. **Composition primitives before new-math primitives, when both are available.** They're lower-risk (no new derivations to verify), they demonstrate the architecture's key claim (composability), and they're educational about what the shipped primitives actually expose.
...
- **State at close**: 159 tests passing. Docs updated, ready to commit.

Next session should pick primitive #8 from the candidates above. EvaluateRebalance (reduced) is the strongest lean — it's the primitive that demonstrates *chaining three primitives* rather than just composing over one.
